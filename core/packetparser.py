...
class PacketParser:
    ...
    def load_pcap(self):
        self.pcap = dpkt.pcap.Reader(open(self.pcap_filepath))
            ...
            for timestamp, buf in self.pcap:
                try:
                    ...
                    if isinstance(ip, dpkt.ip.IP):
                        ...
                        # ip_data = ip_packet.data
                        # print(ip_data)
                        print("pcap路径:{3},时间：{0}，源ip：{1}，目的ip：{2}".format(timestamp,srcip,dstip,self.pcap_filepath))
                    else:
                        continue
                    ip_proto = ip.data
...
# if __name__ == "__main__":
#     file = r'/root/act/detux/pcaps/daf3ccea87a8448f37891443ccc19847f7776671b45bd47a04c5a3db6f540646_1577329785.cap'
#     # file = '/root/ens33.pcap'
#     pk = PacketParser(file)
#     pk.get_network_connections()
