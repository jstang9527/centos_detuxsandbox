...
#        filemagic.close()
    ...
    def qemu_commands(self, platform, sandbox_id):
        if platform == "x86":
            ...
            # sudo qemu-system-i386 -hda qemu/x86/1/debian_wheezy_i386_standard.qcow2 -vnc 127.0.0.1:11 -net nic,macaddr=a0:36:9f:a2:32:c2 -net tap -monitor stdio
    ...
    def ssh_execute(self, host, port, user, password, commands, noprompt = False, logout = True):
        ...
        # print("host=%s;port=%s;user=%s;password=%s;commands=%s" % (host,port,user,password,commands))
        try:
    ...
    def scp(self, host, port, user, password, src_file, dst_file):
        print("host=%s;port=%s;user=%s;password=%s;src_file=%s;dst_file=%s" % (host, port, user, password, src_file, dst_file))
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        try:
            ssh.connect(host, port=port, username=user, password=password)
